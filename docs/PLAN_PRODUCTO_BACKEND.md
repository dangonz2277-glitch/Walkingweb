...
### 2. Cerrar el perímetro del sitio — bloqueante de publicación (SQL aprobado; identidad IP sin validar)
...
Probar acceso directo a la URL de un JS/JSON sin sesión. **Terminado cuando:** una petición anónima no puede obtener el contenido protegido, el proxy bloquea sin fallar (500) ante cookies inválidas, y las redirecciones respetan el host de origen. En `ac9ee3a`, el contador PostgreSQL y los grants pasan pgTAP (48 pruebas) y la suite HTTP. La ruta aún acepta `x-real-ip` como identidad sin que exista prueba de un proxy que la sobrescriba; la suite demuestra que el cliente local puede elegirla. Ver `ANTIGRAVITY_REVISION_06G.md`. Falta verificación en navegador y preview HTTPS antes de publicar.
...
Ejecutar pruebas unitarias, de integración SQL/RLS y E2E de login general, login individual, aislamiento entre perfiles, fecha de medianoche, concurrencia, restablecimiento de contraseña, pérdida de red, importación y respaldo. Revisar el bundle: sin clave general, hashes, tokens ni datos privados. Verificar accesibilidad básica y navegadores principales. **Terminado cuando:** CI verde y el equipo aprueba una prueba con datos anonimizados. Las Órdenes 09 y 10 han sido superadas en remoto: las migraciones se aplicaron a staging, se deshabilitó el registro público en Auth, y un "smoke test" aislado comprobó con éxito logins, conflicto mediante expected_revision obsoleta, restablecimientos, baneo y aislamiento estricto RLS sin corromper el esquema. La concurrencia real simultánea fue validada por la suite separada local, no por ese smoke remoto.
