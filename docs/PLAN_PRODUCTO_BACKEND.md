...
Configurar Auth con registro público deshabilitado (el archivo `config.toml` de producción y la configuración de staging mantendrán `enable_signup = false`, usando una configuración aislada para pruebas). Implementar invitación/alta administrada con clave de servicio, garantizando la creación consistente del Auth user y el perfil; nombre visible, contraseña mínima de ocho caracteres, recuperación y desactivación. Activar controles de contraseñas filtradas y límites de intento disponibles en el proveedor; complementar en gateway si hace falta. Definir caducidad de sesión y reautenticación al abrir Mi Reporte en dispositivo compartido. **Terminado cuando:** hay dos perfiles de prueba independientes y uno desactivado no puede iniciar sesión. Orden 05 validada localmente por HTTP.
