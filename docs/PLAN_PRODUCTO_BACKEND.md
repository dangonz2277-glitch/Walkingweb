...
### 2. Cerrar el perímetro del sitio — bloqueante de publicación (Gateway local aprobado; rate limit SQL no aprobado)
...
Probar acceso directo a la URL de un JS/JSON sin sesión. **Terminado cuando:** una petición anónima no puede obtener el contenido protegido, el proxy bloquea sin fallar (500) ante cookies inválidas, y las redirecciones respetan el host de origen. En `7ee9478`, el contador pasó a PostgreSQL y la suite local pasa, pero la RPC `SECURITY DEFINER` quedó ejecutable por `anon`/`authenticated`, la ruta contiene una clave de servicio de respaldo y la IP sigue derivando de cabeceras no demostradas como confiables. La ventana actual es de 3 segundos, insuficiente para el requisito de 5/minuto. Corregir antes de publicar; ver `ANTIGRAVITY_REVISION_06F.md`.
