...
### 2. Cerrar el perímetro del sitio — bloqueante de publicación (EN PROGRESO - Orden 06)

Elegir hosting que permita una puerta **del lado servidor o edge**. Implementar login general, logout, rotación de clave y sesión segura. Exigir la sesión en `/`, rutas internas, assets, JSON y APIs. Configurar secreto fuera de Git y fuera de cualquier variable `VITE_` pública; HTTPS, límites de intentos y respuesta genérica a fallos. Probar acceso directo a la URL de un JS/JSON sin sesión. **Terminado cuando:** una petición anónima no puede obtener el contenido protegido, aunque conozca la URL exacta.firma/expiración verificadas, logout y pruebas HTTP. Los datos del catálogo aparecen en un chunk JS que ahora se bloquea incluso para el login. **Terminado cuando:** una petición sin sesión válida no puede obtener contenido protegido por ninguna ruta, el login carga sus recursos públicos y una cookie falsa o caducada se rechaza. Ver `ANTIGRAVITY_REVISION_06.md`. No desplegar.
